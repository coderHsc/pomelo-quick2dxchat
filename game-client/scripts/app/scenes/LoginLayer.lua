...
function LoginLayer:ctor()
   ...
   --          if string.len(self.nameEditBox:getText()) < 5 then
	...
	-- --发送消息
 --    local messageData = {username = kLoginNameBox:getText(), password = kLoginPasswordBox:getText(), channel = 1}
 --    MessageManager.sendMessage("LoginAccount", messageData, handler(self, self.loginCallback))
    
 --    --本地保存用户名和密码，下次登陆默认使用
 --    CCUserDefault:sharedUserDefault():setStringForKey("username", kLoginNameBox:getText())
	-- CCUserDefault:sharedUserDefault():setStringForKey("password", kLoginPasswordBox:getText())
end
